...
export const WindowFrame: React.FC<WindowFrameProps & { isFlex?: boolean }> = ({ title, children, className, contentClassName, onClose, isFlex = true }) => {
  return (
    <div className={clsx("bg-white border-2 border-black shadow-neo overflow-hidden flex flex-col", className)}>
      {/* Header */}
      <div className="h-8 bg-white border-b-2 border-black flex items-center px-2 justify-between shrink-0">
      ...
      {/* Content */}
      <div className={clsx("overflow-auto relative", isFlex && "flex-1", contentClassName || "bg-sage-green p-4")}>
        {children}
      </div>
    </div>
  );
